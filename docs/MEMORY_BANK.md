...
| 30 | Rate-Limit Test Bypass (reverted) | Temporarily added a `DISABLE_RATE_LIMIT` env guard in `rateLimit.middleware.ts` to run a full 108-endpoint sweep unthrottled. **Removed on 2026-06-29** — middleware restored to its clean original (all 3 limiters always active). | 2026-06-28 |
